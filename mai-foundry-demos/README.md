...
> **Vendored into MAI_models_playground.** Upstream: [ppiova/mai-foundry-demos](https://github.com/ppiova/mai-foundry-demos)
> ([`v1.1.1`](https://github.com/ppiova/mai-foundry-demos/releases/tag/v1.1.1)).
>
> Deployed via `infra/main.bicep` and tested end-to-end against a live Foundry account. One
> local patch was applied on top of upstream: **`mai/config.py` / `mai/client.py`** now fall back
> to an **Entra ID (Azure AD) bearer token** (`DefaultAzureCredential`, scope
> `https://cognitiveservices.azure.com/.default`) whenever an api-key isn't configured — needed
> because this tenant enforces `disableLocalAuth=true` on Cognitive Services accounts via Azure
> Policy, so no api-key can ever be issued (see `docs/API_VERIFIED.md` for the officially
> documented `Authorization: Bearer <token>` alternative). Readiness flags (`*_ready`) stay
> pure key-presence checks for testability; the Entra fallback only kicks in at the actual HTTP
> call site (`MAIClient._authable` / `MAIClient._auth_header`), so `pytest` still runs 100%
> offline and deterministic.
>
> **Live-tested result (this tenant, Entra ID auth, no api-keys issued):**
>
> | Service | Status | Notes |
> |---|---|---|
> | Thinking-1 (chat) | 🟢 LIVE | Entra ID bearer token accepted on the custom-subdomain endpoint |
> | Image-2.5 (generate) | 🟢 LIVE | Entra ID bearer token accepted on the same endpoint |
> | Transcribe-1.5 | 🟢 LIVE | Entra ID bearer token accepted; round-trips successfully |
> | Voice-2 (TTS) | 🟡 FALLBACK only | The *regional* `<region>.tts.speech.microsoft.com` endpoint returns `401` for an Entra ID bearer token — only an api-key works there, which this tenant's policy blocks |
>
> If your tenant *can* issue api-keys, just fill in `.env` as documented below and every demo
> goes fully 🟢 LIVE, Voice-2 included.
