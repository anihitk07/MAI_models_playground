...
# ponytail: org policy can force disableLocalAuth=true on Cognitive Services
# accounts (no api-key issuance possible). When no key is configured but the
# caller is signed in to Azure (az login / managed identity), fall back to an
# Entra ID bearer token for the same data-plane calls. Cached per-process;
# swap for a token-cache-aware credential if this is ever called at high QPS.
@lru_cache(maxsize=1)
...
def entra_bearer_token() -> str | None:
    try:
        return _entra_credential().get_token("https://cognitiveservices.azure.com/.default").token
    except Exception:
        return None
...
class Config:
    ...
    # ── readiness flags ──────────────────────────────────────────────────────
    # NOTE: these stay pure key-presence checks (no network/credential calls),
    # so they're cheap and deterministic for tests. The Entra ID bearer-token
    # fallback lives only in MAIClient._auth_header at actual call time — a
    # policy-locked account (disableLocalAuth=true, no api-key) can still be
    # LIVE even though these flags read False; that's a readiness label, not
    # an auth guarantee.
    @property
    def foundry_ready(self) -> bool:
        return bool(self.foundry_endpoint) and bool(self.foundry_api_key)

    @property
    def image_ready(self) -> bool:
        return bool(self.image_endpoint) and bool(self.image_api_key)

    @property
    def speech_ready(self) -> bool:
        return bool(self.speech_region) and bool(self.speech_key)

    @property
    def transcribe_ready(self) -> bool:
        return bool(self.speech_endpoint) and bool(self.speech_key)
