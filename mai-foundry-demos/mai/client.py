...
from .config import Config, entra_bearer_token, get_config
...
class MAIClient:
    ...
    # ponytail: single spot every call site routes an auth header through, so
    # the org-policy Entra ID fallback (see config.entra_bearer_token) only
    # needs to be implemented once. Returns {} if neither a key nor a token
    # is available (caller's existing not-ready path already guards that).
    @staticmethod
    def _auth_header(api_key: str, key_header: str) -> dict[str, str]:
        if api_key:
    ...
    @staticmethod
    def _authable(ready: bool, endpoint: str) -> bool:
        """True if the service can actually be called: either an api-key is
        configured (``ready``) or an endpoint is set and an Entra ID token is
        obtainable (org policy can block api-key issuance but the caller is
        signed in via az login / managed identity)."""
        return ready or (bool(endpoint) and entra_bearer_token() is not None)

    # ── Thinking-1 (raw chat; the tool loop lives in demos/thinking_agent.py) ──
    def thinking_ready(self) -> bool:
        return self._authable(self.cfg.foundry_ready, self.cfg.foundry_endpoint)
    ...
    def chat_completion(
        ...
        """
        if not self._authable(self.cfg.foundry_ready, self.cfg.foundry_endpoint):
            raise RuntimeError("Thinking service is not configured")
        resp = requests.post(
            self.cfg.chat_url,
            headers={
                "Content-Type": "application/json",
                **self._auth_header(self.cfg.foundry_api_key, "api-key"),
            },
    ...
    def edit_image(
        ...
        t0 = time.time()
        if self._authable(self.cfg.image_ready, self.cfg.image_endpoint):
            try:
                resp = requests.post(
                    self.cfg.image_url("edits"),
                    headers=self._auth_header(self.cfg.image_api_key, "api-key"),
            ...
            while attempts:
                ...
                try:
                    resp = requests.post(
                        ...
                        headers={
                            "Content-Type": "application/json",
                            **self._auth_header(self.cfg.image_api_key, "api-key"),
                        },
                        json=payload,
                        timeout=self.cfg.image_timeout,
                    )
    ...
    def transcribe(
        ...
        if self._authable(self.cfg.transcribe_ready, self.cfg.speech_endpoint):
            try:
                resp = requests.post(
                    self.cfg.transcribe_url,
                    headers=self._auth_header(self.cfg.speech_key, "Ocp-Apim-Subscription-Key"),
    ...
    def synthesize(
        ...
        if self._authable(self.cfg.speech_ready, self.cfg.speech_endpoint):
            try:
                resp = requests.post(
                    self.cfg.tts_url,
                    headers={
                        ...
                        **self._auth_header(self.cfg.speech_key, "Ocp-Apim-Subscription-Key"),
                    },
